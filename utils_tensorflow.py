...
def set_random_seeds(seed):
    tf.set_random_seed(seed) #Define a semente aleatória no nível do gráfico.
    np.random.seed(seed) # Semeie o gerador. Este método é chamado quando é inicializado. Pode ser chamado novamente para repagar o gerador
    random.seed(seed)
...
# definindo a função que cria o aprendizado
def make_lr(lr_args, step_counter):
    initial_lr = tf.constant(lr_args['initial']) # Cria um tensor constante. O tensor resultante é preenchido com valores do tipo dtype, conforme especificado pelos argumentos valuee (opcionalmente) shape(veja os exemplos abaixo).
    schedule = lr_args['schedule']
    if schedule == 'constant':
        lr = initial_lr
    elif schedule == 'linear':
        assert type(step_counter) == tf.Variable
        steps = tf.cast(step_counter, tf.float32) #Lança um tensor (step_counter) para um novo tipo (tf.float32).
        zero_by_steps = tf.cast(lr_args['zero_by_steps'], tf.float32)
        lr = initial_lr * (1 - steps / zero_by_steps)
        lr = tf.clip_by_value(lr, clip_value_min=0.0, clip_value_max=float('inf')) # Prende os valores de tensor a um mínimo e máximo especificados.
    else:
