...
class Timer:
    """
    Uma simples aula de timer.
     * Defina a duração do cronômetro com o argumento `duration_seconds` para o construtor.
     * Inicie o cronômetro chamando `reset ()`.
     * Verifique se o cronômetro é feito chamando `done ()`.
    """
...
class TensorFlowCounter:
    """
    Contador implementado como uma variável TensorFlow no gráfico da sessão fornecida.
    Útil se você quiser que o valor seja inserido em alguma outra operação, por exemplo cálculo da taxa de aprendizagem.
    """
